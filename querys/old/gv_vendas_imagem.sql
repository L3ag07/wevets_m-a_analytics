...
DECLARE @DataFinal AS DATE = '2025-05-01';

-- Create a temporary table for CardioSections to avoid the LIKE operation in the main query
DECLARE @ImagemSections TABLE (Id INT PRIMARY KEY);
INSERT INTO @CardioSections
SELECT Id 
FROM GV_SeccaoProduto 
WHERE Descricao LIKE '%Imagem%';
...
WHERE cdv.Documento = 'FAT'
  ...
  AND p.IdSeccao IN (SELECT Id FROM @ImagemSections )
